...
![image-20190808152021768](/Users/xygj/Library/Application Support/typora-user-images/image-20190808152021768.png)
...
在`_trimInBackground`中为什么没有使用`__weak`和`__strong`去修饰self呢? 这一块还没搞清楚, 希望看到的大神指点下. 猜想: 是因为在`_trimRecursively`的block中已经self进行了weak和strong修饰, `_trimInBackground`是由`strongSelf`调用的.`_trimInBackground`中的self可以理解为strongSelf.
